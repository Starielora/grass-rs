...
void main()
{
    ...
    if (IS_WIREFRAME) {
        ...
        color = gl_VertexIndex >= 24
            ...
            : push_constants.frustum_colors.edges_color;
    } else {
        corner_idx = solid_indices[gl_VertexIndex];
        int face_idx = gl_VertexIndex / 6;
        float alpha = face_idx < 2 ? 0.0 : push_constants.frustum_colors.planes_color.a;
        color = vec4(push_constants.frustum_colors.planes_color.rgb, alpha);
    }
